...
| Unnamed top-level envelope that declares a `payload.type`, carries a `payload.item`, has a payload that is not a plain object, or carries `content`, `text`, `message`, `phase` or `role` | Fails closed as `schema-drift`. A declared payload type makes the record an envelope/payload **pair**, and an unknown pair is what `schema-drift` has always meant; the other three shapes could each hold a reply body the reader would drop while certifying the turn. |

...
| Wire contract | `version` is per-method and is matched **exactly, before ownership is evaluated**. `thread-follower-start-turn` = **2**; the payload key is `params.turnStart` = `{request:{threadId,turnTrigger,input[]}, context?}`; `request.threadId` must equal `params.conversationId`; the frame carries **no `hostId` key** (any non-null frame `hostId` raises every `thread-follower-*` method's required version by one, so the key must be absent, not null) and no `turnStart.context.responseItems`. The full method-version table is checked in at `tests/fixtures/codex_desktop_method_versions.json` and asserted by `tests/test_router_contract.sh`, which fails on drift instead of pinning the values |
...
| Live status of this contract | **Accepted live by the app on `26.901.4073.0` from a working tree of this branch; NOT accepted through the installed route.** Four separate facts, which must not be collapsed into one another. **(1) Direct-harness proof — performed.** On 2026-09-05 a run of `skills/ipc/scripts/codex_ipc_write_proof.mjs` from a working tree of this branch sent one version-2 `thread-follower-start-turn`; the app answered `resultType:"success"`, the turn id resolved to a single candidate, the target thread's own rollout then carried the agent acknowledgement followed by a terminal `task_complete`, and the reader bound the proof to that turn id (`verificationStatus: turn-bound`). The frame described in this table is therefore accepted by the running app and does start a real turn. **(2) Wrapper delivery to an already-owned thread — performed, and narrower than it looks.** The same day, `skills/ipc/scripts/handoff_to_codex.sh --ipc`, also run from a working tree of this branch, delivered into a thread the renderer **already owned** (`RESULT: gui-delivered -- reason=renderer-owned -- confirmation=rollout-hit`, a bounded pickup observation rather than a completion proof); that thread's rollout subsequently recorded a terminal `task_complete`. Because the target was already loaded and owned, the autoload, foreground-switch and deep-link paths were **not** exercised at version 2 and remain unproven. **(3) Pre-tag delivery smoke from the merged primary checkout — NOT performed.** It is ordered before the tag and before propagation, and no working-tree run substitutes for it. **(4) Acceptance through the normal installed route — NOT performed.** As of 2026-09-05, every installed root still ships `FOLLOWER_START_TURN_VERSION = 1`; an installed-wrapper attempt on 2026-09-05 admitted nothing (`RESULT: gui-unowned -- reason=autoload-incomplete -- confirmation=not-attempted`, target rollout byte-identical). Merging this branch, or moving the default branch, does not update the installed clients — only propagation does — so installed-route delivery is claimed only after a wrapper attempt over the **installed** client has run and been certified independently |
